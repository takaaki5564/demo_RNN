# basic RNN

import numpy as np
import chainer
from chainer import cuda, Function, gradient_check, Variable, \
                    optimizers, serializers, utils
from chainer import Link, Chain, ChainList
import chainer.functions as F
import chainer.links as L

# set data

vocab = {}

def load_data(filename):
    global vocab #各単語のidを設定
    words = open(filename).read().replace('\n','<eos>').strip().split() #空白を削除
    dataset = np.ndarray((len(words),), dtype=np.int32)
    for i, word in enumerate(words):
        if word not in vocab:
            vocab[word] = len(vocab)
        dataset[i] = vocab[word]
    return dataset #単語列

train_data = load_data('ptb.train.txt')
eos_id = vocab['<eos>']

class MyLSTM(chainer.Chain):
    def __init__(self, v, k):
        super(MyLSTM, self).__init__(
            embed = L.EmbedID(v, k),
            Wz = L.Linear(k,k), #RNN
            Wi = L.Linear(k,k), #Input Gate
            Wf = L.Linear(k,k), #Forget Gate
            Wo = L.Linear(k,k), #Output Gate
            Rz = L.Linear(k,k),
            Ri = L.Linear(k,k),
            Rf = L.Linear(k,k),
            Ro = L.Linear(k,k),
            W = L.Linear(k,v),  #Output
        )
    def __call__(self, s): #s:辞書単語のid配列
        accum_loss = None
        v, k = self.embed.W.data.shape
        h = Variable(np.zeros((1,k), dtype = np.float32)) #History Input
        c = Variable(np.zeros((1,k), dtype = np.float32)) #Input
        for i in range(len(s)):
            next_w_id = eos_id if (i==len(s)-1) else s[i+1]
            #教師データは次(t+1)の単語
            tx = Variable(np.array([next_w_id], dtype=np.int32))
            #k次元分散表現に圧縮
            x_k = self.embed(Variable(np.array([s[i]], dtype=np.int32)))
            
            #活性化関数はRNN,出力層はtanh,その他はsigmoid
            z0 = self.Wz(x_k) + self.Rz(h)
            z1 = F.tanh(z0)                 #result of RNN layer 
            i0 = self.Wi(x_k) + self.Ri(h)
            i1 = F.sigmoid(i0)              #result of input layer
            f0 = self.Wf(x_k) + self.Rf(h)
            f1 = F.sigmoid(f0)              #result of forget layer
            c = i1 * z1 + f1 * c
            o0 = self.Wo(x_k) + self.Ro(h)  #output layer
            o1 = F.sigmoid(o0)
            h = o1 * F.tanh(c)
            loss = F.softmax_cross_entropy(self.W(h), tx)
            accum_loss = loss if accum_loss is None else accum_loss + loss
        return accum_loss

demb = 100
model = MyLSTM(len(vocab), demb)
optimizer = optimizers.Adam()
optimizer.setup(model)

for epoch in range(5):
    s = []
    for pos in range(len(train_data)):
        id = train_data[pos]
        s.append(id)
        if (id == eos_id):
            model.zerograds()
            loss = model(s)
            loss.backward()
            if (len(s) > 29): #文字数30以上の文の場合、文字を忘れる
                loss.unchain_backward() #truncate
            optimizer.update()
            s = []
        if (pos % 100 == 0):
            print ("{0}/{1} finished".format(pos, len(train_data)))
    outfile = "lstm0-" + str(epoch) + ".model"
    serializers.save_npz(outfile, model)
            

